...
export default function Tracking() {
  ...
  const handleEdit = (trackerId, updatedTrackerData) => {
    setTrackers((prevTracker) =>
      prevTracker.map((item) => (item._id === trackerId ? updatedTrackerData : item))
    );
    ...
    return (
      <Box>
        ...
        <Button onClick={onOpen} mt={4}>Add Tracking</Button>
        <Modal isOpen={isOpen} onClose={onClose}>
          <ModalOverlay />
          <ModalContent>
            <ModalHeader fontSize="40px" fontWeight="bold">Tracking</ModalHeader>
            <ModalCloseButton />
            <ModalBody>
              <Stack spacing={10}>
                <input
                  ...
                  onChange={(e) => setNewTracker({ ...newTracker, bodyWeight: e.target.value })}
                ...
                <input
                  ...
                  onChange={(e) => setNewTracker({ ...newTracker, steps: parseInt(e.target.value) })}
                  style={{ backgroundColor: 'PaleTurquoise', height: '35px', borderRadius: '5px' }}
                />
                <input
                  type="text"
                  ...
                  onChange={(e) => setNewTracker({ ...newTracker, duration: e.target.value })}
                  style={{ backgroundColor: 'PaleTurquoise', height: '35px',  borderRadius: '5px' }}
                />
                <input
                  type="text"
                  ...
                  onChange={(e) => setNewTracker({ ...newTracker, kcals: e.target.value })}
                  style={{ backgroundColor: 'PaleTurquoise', height: '35px',  borderRadius: '5px'}}
                />
              </Stack>
            </ModalBody>
